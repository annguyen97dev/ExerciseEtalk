...
var PopOver = /*#__PURE__*/ (function (_Component) {
	...
	function PopOver(props) {
		...
		_defineProperty(_assertThisInitialized(_this), 'parseItems', function (
			items,
		) {
			if (items != null && items != undefined && items.length > 0) {
				...
				items.forEach(function (item, index) {
					var handler = "<span class='popupItem' data-action=\""
						.concat(index, '"  id="')
						.concat(item.text, '-')
						.concat(index, '">\n            ')
						.concat(item.text, '\n          </span>');
		...
		_defineProperty(
			...
			function (e) {
				...
				if (t.toString().length !== 0) {
					...
					console.log('RANGE MOUSEUP: ', _this.range);

					// var saveNode = _this.range.startContainer;

					// var startOffset = _this.range.startOffset; // where the range starts
					// var endOffset = _this.range.endOffset; // where the range ends
					...
					// console.log('NODE TAG NAME: ', nodeTagName);
					...
					if (!_this.checkMobile()) {
						var top = 0,
							left = 0;
						if (e.pageY < _this.state.startY) top = e.pageY - 50;
						else top = _this.state.startY - 50;
		...
		_defineProperty(
			...
			function (e) {
				...
				console.log('RANGE MOUSEDOWN: ', _this.range);

				// console.log('CLICKED: ', t);

				if (
					t.toString().length !== 0 &&
					e.target.classList.contains('popupItem')
				) {
					if (_this.frangment) {
						if (_this.props.colorText) {
							...
							try {
								...
								if (
									_this.frangment &&
									_this.frangment.childNodes &&
									_this.frangment.childNodes.length
								) {
									_this.frangment.childNodes.forEach(function (child, i) {
										tagCollection.forEach(function (tag) {
											if (
												...
												spanWrapper.className = 'extracted-simple-text id-'.concat(
													id,
												);
												spanWrapper.id = 'id-'.concat(id);
												spanWrapper.append(child.textContent);
												var Html = tag.innerHTML.replace(
													child.textContent,
												...
												tag.innerHTML &&
												tag.innerHTML.includes(child.innerHTML)
											) {
												var _spanWrapper = document.createElement('span');

												_spanWrapper.className = 'extracted-simple-text id-'.concat(
													id,
												);
												...
												var _Html = tag.innerHTML.replace(
													child.innerHTML,
													_spanWrapper.outerHTML,
												);
						...
						_this.props.events[e.target.dataset.action].handler(
							frangmentWrapper,
							_this.range.toString(),
							id,
						);
		...
		_defineProperty(_assertThisInitialized(_this), 'tooltip', function (id) {
			if (!_this.tooltipElement) {
				...
				button.id = 'tooltip';
				button.textContent = _this.props.unmarkText
					? _this.props.unmarkText
					: 'remove';
		...
		_defineProperty(_assertThisInitialized(_this), 'deleteElement', function (
			evt,
		) {
		...
		_defineProperty(_assertThisInitialized(_this), 'handleLost', function (e) {
			...
			if (
				...
				(e.target.id === 'tooltip' &&
					e.relatedTarget.className === 'extracted-simple-text')
			) {
				//console.log('tooltip')
			} else {
				console.log('other');
